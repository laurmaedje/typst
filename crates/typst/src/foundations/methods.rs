...
use crate::foundations::{Args, Array, Dict, IntoValue, NoneValue, Str, Type, Value};
...
pub(crate) fn call_method_mut(
    ...
    if let Some(array) = value.to_mut::<Array>() {
        match method {
    ...
    } else if let Some(dict) = value.to_mut::<Dict>() {
        match method {
...
pub(crate) fn call_method_access<'a>(
    ...
    let slot = if value.is::<Array>() {
        let array = value.to_mut::<Array>().unwrap();
        match method {
        ...
        let dict = value.to_mut::<Dict>().unwrap();
        match method {
