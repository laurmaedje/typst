...
impl Collector {
    ...
    fn collect(mut self) -> Result<Vec<Test>, Vec<TestParseError>> {
        for entry in walkdir::WalkDir::new(crate::SUITE_PATH).sort_by_file_name() {
            ...
            // Skip modules intended for importing.
            let lossy = path.to_string_lossy();
            if lossy.contains("compiler")
                && (lossy.ends_with("module.typ") || lossy.contains("modules"))
            {
                continue;
            }

            let text = std::fs::read_to_string(path).unwrap();
            Parser::new(&mut self, path, &text).parse();
        }
...
impl<'a> Parser<'a> {
    ...
    fn parse(&mut self) {
        ...
        while !self.s.done() {
            ...
            self.collector.tests.push(Test {
                pos: FilePos::new(self.path, self.test_start_line),
                name,
                source,
                notes,
                skipped: false,
    ...
    fn parse_position(&mut self, source: &Source) -> Option<usize> {
        ...
        let column_idx = usize::try_from(column).ok()?.checked_sub(1)?;
...
fn filtered(name: &str) -> bool {
    ...
    filter.is_empty() || filter.iter().any(|v| name.contains(v))
}
...
impl Display for TestParseError {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        write!(f, "{} ({}:{})", self.message, self.path.display(), self.line)
    }
}
